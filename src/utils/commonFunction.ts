...
export const formatJsDateToSqlDateTime = (
  dateTime: string | null = null
): string => {
  const utcDate = dateTime ? new Date(dateTime as string) : new Date();
  return `${utcDate.getUTCFullYear()}-${padLeft(
    utcDate.getUTCMonth() + 1
  )}-${padLeft(utcDate.getUTCDate())} ${padLeft(
    utcDate.getUTCHours()
  )}:${padLeft(utcDate.getUTCMinutes())}:${padLeft(utcDate.getUTCSeconds())}`;
};
